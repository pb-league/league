...
function getRegistrySheet() {
  ...
  if (!sheet) {
    ...
    sheet.getRange(1, 1, 1, 14).setValues([['leagueId', 'name', 'sheetId', 'active', 'canCreateLeagues', 'hidden', 'createdDate', 'expiryDays', 'maxPlayers', 'maxCourts', 'maxRounds', 'maxSessions', 'customerId', 'adminEmail']]);
...
function sendFeedback(body) {
  ...
  const { feedbackType, name, email, message, leagueId, leagueName } = body;

  if (!message || !message.trim()) return { success: false, error: 'Message is required.' };

  const fromLabel = name ? name : 'Anonymous';
  const subject = `PB League Feedback [${feedbackType || 'Other'}] from ${fromLabel}`;

  const html = `<div style="font-family:sans-serif; max-width:580px; color:#222;">
    <h2 style="color:#2d7a3a; margin-bottom:4px;">League Manager Feedback</h2>
    <hr style="border:none; border-top:1px solid #ddd; margin:12px 0;">
    <table style="width:100%; font-size:0.95rem; border-collapse:collapse;">
      <tr><td style="padding:6px 12px 6px 0; color:#666; width:140px;">Type</td>
          <td style="padding:6px 0; font-weight:600;">${feedbackType || 'Other'}</td></tr>
      <tr style="background:#f9f9f9;"><td style="padding:6px 12px 6px 0; color:#666;">From</td>
          <td style="padding:6px 0;">${fromLabel}</td></tr>
      <tr><td style="padding:6px 12px 6px 0; color:#666;">Email</td>
          <td style="padding:6px 0;">${email || '(not provided)'}</td></tr>
      <tr style="background:#f9f9f9;"><td style="padding:6px 12px 6px 0; color:#666;">League</td>
          <td style="padding:6px 0;">${leagueName || leagueId || '—'}</td></tr>
    </table>
    <div style="margin-top:16px; background:#f5f5f5; border-radius:6px; padding:14px 16px;
      font-size:0.95rem; line-height:1.7; white-space:pre-wrap;">${message.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
    <hr style="border:none; border-top:1px solid #ddd; margin:16px 0 10px;">
    <p style="font-size:0.8em; color:#aaa;">Sent from Pickleball League Manager feedback form</p>
  </div>`;

  try {
    const opts = { htmlBody: html, name: 'PB League Feedback' };
    if (email) opts.replyTo = email;
    GmailApp.sendEmail(DEV_EMAIL, subject, '', opts);
  } catch(e) {
...
function getLeagueList(includeHidden) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    leagues.push({
      ...
      adminEmail:       data[i][13] ? String(data[i][13]).trim() : null,
    });
  }
  return leagues;
}
...
function doPost(e) {
  ...
  try {
    if (action === 'submitApplication') {
      ...
      result = updateLeague(body.leagueId, body.name, body.sheetId, body.active, body.canCreateLeagues, body.hidden, body.callerLeagueId, body.limits, body.adminEmail);
    } else {
      ...
      switch (action) {
        ...
        case 'sendFeedback':       result = sendFeedback(body); break;
        default: result = { error: 'Unknown action: ' + action };
      }
    }
  } catch (err) {
...
function updateLeague(leagueId, name, sheetId, active, canCreateLeagues, hidden, callerLeagueId, limits, adminEmail) {
  const sheet = getRegistrySheet();
  const data = sheet.getDataRange().getValues();
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === String(leagueId)) {
      ...
      if (adminEmail !== undefined) sheet.getRange(i + 1, 14).setValue(adminEmail || '');
      return { success: true };
    }
  }
...
function validatePIN(ss, name, pin) {
  ...
  if (player.active === 'pend') return { valid: false, reason: 'Your account is pending approval. You will receive an email once approved.' };
  if (player.active === false) return { valid: false, reason: 'This account is inactive.' };
  const storedPin = String(player.pin || '').trim();
  const noPin = storedPin === '' || storedPin === 'undefined' || storedPin === 'null';
  const match = noPin || storedPin === String(pin || '').trim();
  if (match) {
    const role = player.role || (player.canScore ? 'scorer' : 'player');
    const isAdmin = (role === 'admin' || role === 'manager');
    return { valid: true, name: player.name, role, isAdmin, canScore: role === 'scorer' || isAdmin || role === 'assistant', noPin };
  }
...
function registerPlayer(ss, body) {
  ...
  const config = getConfig(ss).config;
  if (!config.allowRegistration || config.allowRegistration === 'false' || config.allowRegistration === '0') return { success: false, error: 'Registration is not enabled for this league.' };
...
function getAllData(ss, sinceWeek) {
  ...
  // Look up this league's limits from the registry so the frontend can enforce them
  let limits = null;
  try {
    const leagueId = ss.getId ? null : null; // ss is the spreadsheet object
    // Find the registry entry by sheetId
    const regSheet = getRegistrySheet();
    const regData  = regSheet.getDataRange().getValues();
    const ssId     = ss.getId();
    const intOrNull = v => (v !== '' && v !== null && !isNaN(parseInt(v))) ? parseInt(v) : null;
    for (let i = 1; i < regData.length; i++) {
      if (String(regData[i][2]) === ssId) {
        limits = {
          expiryDays:  intOrNull(regData[i][7]),
          maxPlayers:  intOrNull(regData[i][8]),
          maxCourts:   intOrNull(regData[i][9]),
          maxRounds:   intOrNull(regData[i][10]),
          maxSessions: intOrNull(regData[i][11]),
        };
        // Check expiry
        const createdDate = regData[i][6] ? new Date(regData[i][6]) : null;
        if (createdDate && limits.expiryDays !== null) {
          const expiryMs = limits.expiryDays * 24 * 60 * 60 * 1000;
          limits.expired = (Date.now() - createdDate.getTime()) > expiryMs;
        } else {
    ...
    Logger.log('getAllData limits lookup failed: ' + e);
  }
